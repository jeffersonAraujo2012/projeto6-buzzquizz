function criarQuizz() {
  ...
  promise.then(res => {
    let meusQuizzes = localStorage.getItem("id-meu-quizz");
    if (meusQuizzes) {
...
function viewSucesso(quizzCriado) {
  ...
  criarQuizzSucesso.innerHTML = /*HTML*/`
    <div class="criar-quizz criar-quizz--sucesso">
      ...
      <div class="card-quiz card-quiz--sucesso" style="background-image: url('${quizzCriado.image}');">
        <h3 class="card-quiz__title">${quizzCriado.title}</h3>
      </div>
      <button type="submit" class="btnProsseguir btnProsseguir--sucesso">
        Acessar Quizz
      </button>
